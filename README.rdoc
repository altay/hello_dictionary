...
Wornik API Documentation: http://docs.wordnik.com/api/methods

Wordnik API Key Signup: http://api.wordnik.com/signup/

Wordnik API Support: http://groups.google.com/group/wordnik-api

The wordnik-ruby gem -- the official Ruby gem for the Wordnik API -- is on github: http://github.com/wordnik/wordnik-ruby
...
3. Edit two lines to hello_dictionary/config/environment.rb:
  Add the wordnik-ruby gem (inside of the Rails::Initializer.run block):
    config.gem "wordnik-ruby"
  Add your Wordnik API key at the end of the file
...
4. Create a controller and add an 'index' action:
  ...
  Edit the new controller (hello_dictionary/app/controllers/wordnik_controller.rb), like so:
...
5. Create your app's layout (hello_dictionary/app/views/layouts/application.html.erb):
    <html>
      <head>
    ...
    <div>
      ...
      <h3>The Wordnik Word of the Day for <%= Time.now.strftime("%A, %B %d, %Y") %> is... <strong><%= link_to(@wotd['wordstring'], lookup_url(:word_search=>@wotd['wordstring'])) %></strong>!
      </h3>
      <p>
...
6. Create the template for the wordnik/index action (hello_dictionary/app/views/wordnik/index.html.erb):
    <% unless @word.nil? %>
      <hr />
      <div>
              ...
              <% unless related_words.blank? -%>
                ...
                <ul>
                  ...
                  <% related_words[0..4].each do |related_word| %>
                    <li><%= link_to(related_word.wordstring, lookup_url(:word_search=>related_word.wordstring)) -%></li>
                  <% end %>
                </ul>
              <% end %>
            <% end %>
          ...
          <ul>
            <% @phrases.each do |phrase| %>
              <% unless phrase['gram1'].blank? -%>
                ...
                <li><%= link_to(complete_phrase, lookup_url(:word_search=>complete_phrase)) %></li>
              <% end %>
            <% end %>
          </ul>
...
8. Navigate to http://localhost:3000/wordnik
